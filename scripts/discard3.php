...
function discard($site_id)
{
    ...
    // Discard links
    // $negatives = $db->get_col("select  link_id from links, sub_statuses WHERE id = $site_id and date > $min_date and status = 'queued' and link_id = link and link_karma < 0 and (link_date < $max_date or link_karma < -100) and (link_karma < -link_votes*2 or (link_negatives > 20 and link_negatives > link_votes/2)) and (link_negatives > 20 or (link_negatives > 4 and link_negatives > link_votes+3) ) order by link_id asc");
    // Simlified version
    $negatives = $db->get_col("select  link_id from links, sub_statuses WHERE id = $site_id and date > $min_date and status = 'queued' and link_id = link and link_karma < -20 and link_negatives > 5 and link_negatives > link_votes + 2 and (link_date < $max_date or link_karma < -100) order by link_id asc");
...
function punish_comments($hours = 2)
{
    ...
    $users = "SELECT  distinct comment_user_id as user_id from comments, users WHERE comment_date > from_unixtime($comments_from) and comment_karma < -70 and comment_user_id = user_id and user_level != 'disabled' and user_karma >= $min_karma";
    ...
    foreach ($result as $dbuser) {
        ...
        $comment_votes_count = (int) $db->get_var("SELECT COUNT(*) from votes, comments WHERE comment_user_id = $user->id and comment_date > from_unixtime($comments_from) and vote_type='comments' and vote_link_id = comment_id and  vote_date > from_unixtime($votes_from) and vote_user_id != $user->id");
        if ($comment_votes_count > 5) {
            $votes_karma = (int) $db->get_var("SELECT  sum(vote_value) from votes, comments WHERE comment_user_id = $user->id and comment_date > from_unixtime($comments_from) and vote_type='comments' and vote_link_id = comment_id and vote_date > from_unixtime($votes_from) and vote_user_id != $user->id");
            if ($votes_karma < 50) {
                $distinct_votes_count = (int) $db->get_var("SELECT COUNT(distinct comment_id) from votes, comments WHERE comment_user_id = $user->id and comment_date > from_unixtime($comments_from) and vote_type='comments' and vote_link_id = comment_id and  vote_date > from_unixtime($votes_from) and vote_user_id != $user->id");
                $comments_count = (int) $db->get_var("SELECT COUNT(*) from comments WHERE comment_user_id = $user->id and comment_date > from_unixtime($comments_from)");
...
function depublish($site_id)
{
    ...
    $links = $db->get_col("select  link_id as id from links, sub_statuses WHERE id = $site_id and status = 'published' and date > DATE_SUB(NOW(), INTERVAL $days day) and date < DATE_SUB(NOW(), INTERVAL 14 MINUTE) and link = link_id and link_negatives > link_votes / 5");

    if ($links) {
        $votes_clicks = $db->get_col("select  link_votes/counter from links, sub_statuses, link_clicks WHERE sub_statuses.id = $site_id and status = 'published' and date > DATE_SUB(NOW(), INTERVAL $days day) and link = link_id and link_clicks.id = link");
        sort($votes_clicks);
        ...
        foreach ($links as $link) {
            ...
            $negatives = (int) $db->get_var("select  sum(user_karma) from votes, users WHERE vote_type='links' and vote_link_id=$l->id and vote_date > from_unixtime($l->date) and vote_date > DATE_SUB(NOW(), INTERVAL 24 HOUR) and vote_value < 0 and vote_user_id > 0 and user_id = vote_user_id and user_karma > ".$globals['depublish_negative_karma']);
            $positives = (int) $db->get_var("select  sum(user_karma) from votes, users WHERE vote_type='links' and vote_link_id=$l->id and vote_date > from_unixtime($l->date) and vote_value > 0 and vote_date > DATE_SUB(NOW(), INTERVAL 24 HOUR) and vote_user_id > 0 and user_id = vote_user_id and user_karma > ".$globals['depublish_positive_karma']);
