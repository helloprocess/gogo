...
my $sql = "select  user_id, count(*) as votes from users, votes where vote_type='links' and user_id = vote_user_id and vote_date > FROM_UNIXTIME($from) group by user_id";
...
sub crossed_votes {
	...
	#	print "$userid, $user_votes{$userid}: ";
		# Each voted link
		$sql = "select  vote_link_id, vote_value from votes where vote_user_id=$userid and vote_date > FROM_UNIXTIME($from)";
		$sth = $dbh->prepare($sql);
		$sth->execute();
		while( ($linkid, $value) = $sth->fetchrow_array ) {
			...
			$sql = "select distinct  vote_user_id, vote_value from votes where vote_type='links' and vote_link_id = $linkid and vote_user_id > 0 and vote_user_id != $userid and vote_date > FROM_UNIXTIME($from)";
