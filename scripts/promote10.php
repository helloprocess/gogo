...
function promote($site_id) {
	...
	$past_karma_long = intval($db->get_var("SELECT  avg(karma) from sub_statuses WHERE id = $site_id and date >= date_sub(now(), interval 7 day) and status='published'"));
	$past_karma_short = intval($db->get_var("SELECT  avg(karma) from sub_statuses WHERE id = $site_id and date >= date_sub(now(), interval 12 hour) and status='published'"));
	...
	$globals['users_karma_avg'] = (float) $db->get_var("select  avg(link_votes_avg) from links, sub_statuses where id = $site_id and status = 'published' and date > date_sub(now(), interval 72 hour) and link_id = link");
