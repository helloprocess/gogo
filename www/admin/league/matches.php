...
if (!empty($_POST['process'])) {
    ...
    if (is_numeric($_POST['vote_starts'])) {
     ...
     if ($valid && empty($_POST['match_id'])) {
        GameMatch::create($_POST);
    } elseif ($valid) {
        $league = new GameMatch($_POST['match_id']);
        if (!$league->read()) {
...
case 'list':
default:
    $total    = $db->get_row("SELECT count(*) as total from " . GameMatch::TABLE)->total;
    $per_page = 30;
