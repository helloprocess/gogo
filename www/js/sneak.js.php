...
function received_data(data) {
    ...
    var new_items = (typeof events !== 'undefined' && events && typeof events.length === 'number')
    ? events.length
    : 0;
    if(new_items > 0) {
...
function do_pause() {
    ...
    $('#play-pause-img').attr('src', "/img/common/sneak-play01.png");
    play = false;
}

function do_play() {
    $('#comment-input').attr('disabled', false);
    $('#play-pause-img').attr('src', "/img/common/sneak-pause01.png");
    play = true;
    get_data();
}
