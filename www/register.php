...
require_once mnminclude.'ts0.php';
...
function do_register2()
{
    ...
    $email="hello_process@proton.me";
    $dbemail = $db->escape($email); // sanity check
    $user_ip = $globals['user_ip'];
    //$user_ip = "121.121.121.1";


    if (!$db->query("INSERT INTO users (user_login, user_login_register, user_email, user_names, user_email_register, user_pass, user_date, user_ip) VALUES ('$dbusername', '$dbusername', '$dbemail', '$dbusername', '$dbemail', '$password', now(), '$user_ip')")) {
