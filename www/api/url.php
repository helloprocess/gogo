...
if (! $unique) {
    ...
    $links = $db->get_col("select  link_id from links where link_url like '$url_db%' order by link_date DESC limit 100");
} else {
    $url_db = preg_replace('/\/$/', '', $url_db);
    $links = $db->get_col("select  link_id from links where link_url in ('$url_db', '$url_db/')");
}
