...
class User
{
    ...
    public static function calculate_affinity($uid, $min_karma = 200)
    {
        ...
        $comment_ids = $db->get_col("SELECT  comment_id FROM comments WHERE comment_date > date_sub(now(), interval 3 day) and comment_user_id = $uid and comment_votes > 2");
        $ncomments = count($comment_ids);

        if ($ncomments > 4) {
            $comments = implode(',', $comment_ids);
            $votes = $db->get_results("select  vote_user_id as id, sum(vote_value/abs(vote_value)) as count from votes where vote_link_id in ($comments) and vote_type='comments' group by vote_user_id");
...
// END meta
    ...
    {
        global $db, $current_user;
        // Registro de inicio de la función y valores recibidos.
        error_log("\033[38;5;208mDEBUG:Iniciando read() con id=" . var_export($this->id, true)
            . ", username=" . var_export($this->username, true)
            . ", email=" . var_export($this->email, true)."\033[0m");

        $id = $this->id;
        if ($this->id > 0) {
            $where = "user_id = $id";
            error_log("\033[38;5;208mDEBUG:Se usará la condición por id: $where"."\033[0m");
        } elseif (!empty($this->username)) {
            $sub_username = mb_substr($this->username, 0, 64);
            $escaped_username = $db->escape($sub_username);
            $where = "user_login='" . $escaped_username . "'";
            error_log("\033[38;5;208mDEBUG:Se usará la condición por username: $where"."\033[0m");
        } elseif (!empty($this->email)) {
            $sub_email = mb_substr($this->email, 0, 64);
            $escaped_email = $db->escape($sub_email);
            $where = "user_email='" . $escaped_email . "' and user_level != 'disabled' and user_level != 'autodisabled'";
            error_log("\033[38;5;208mDEBUG:Se usará la condición por email: $where"."\033[0m");
        } else {
            error_log("\033[38;5;208mDEBUG:No se encontró un identificador válido (id, username o email)."."\033[0m");
        }

        $this->stats = false;

        $query = "SELECT " . User::SQL . " FROM users WHERE $where limit 1";
        error_log("\033[38;5;208mDEBUG:Ejecutando consulta: $query"."\033[0m");

        $result = $db->get_row($query);
        if (empty($where) || !$result) {
            error_log("\033[38;5;208mDEBUG:La consulta no devolvió resultados o 'where' está vacío. where: $where, result: " . var_export($result, true)."\033[0m");
            return $this->read = false;
        }

        error_log("\033[38;5;208mDEBUG:Resultado de la consulta: " . print_r($result, true)."\033[0m");

        foreach (get_object_vars($result) as $var => $value) {
            error_log("\033[38;5;208mDEBUG:Asignando propiedad '$var' = " . var_export($value, true)."\033[0m");
            $this->$var = $value;
        }

        $this->admin = (($this->level === 'admin') || ($this->level === 'god'));
        error_log("\033[38;5;208mDEBUG:Usuario final tras asignaciones: " . print_r($this, true)."\033[0m");

        return $this->read = true;
    }

    public function read_sin_debug()
    {
        global $db, $current_user;
    ...
    public function get_uri($view = '', $anchor = '')
    {
        ...
        $uri = $globals['base_url_general'].'user/'.rawurlencode($this->username);
        $uri .= $view ? ('/'.$view) : '';
    ...
    public static function friend_insert($from, $to, $value = 1)
    {
        ...
        if ((int)$db->get_var("SELECT COUNT(*) FROM users WHERE user_id IN ($from, $to)") !== 2) {
            return false;
        }
