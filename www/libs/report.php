...
class Report extends LCPBase
{
    ...
    const SQL_COMMENT_GROUPED = '
        SELECT 
            COUNT(*) AS report_num, 
            MIN(report_id) AS id, 
            MIN(report_type) AS type, 
            MIN(report_date) AS date, 
            MIN(report_modified) AS modified, 
            MIN(report_status) AS status, 
            report_reason AS reason, 
            MIN(reporters.user_id) AS reporter_id, 
            MIN(reporters.user_level) AS reporter_user_level, 
            MIN(reporters.user_login) AS reporter_user_login, 
            MIN(authors.user_id) AS author_id, 
            MIN(authors.user_level) AS author_user_level, 
            MIN(authors.user_login) AS author_user_login, 
            MIN(revisors.user_id) AS revisor_id, 
            MIN(revisors.user_level) AS revisor_user_level, 
            MIN(revisors.user_login) AS revisor_user_login, 
            MIN(report_ip) AS ip, 
            comment_id AS ref_id, 
            MIN(comment_order) AS comment_order, 
            MIN(comment_link_id) AS comment_link_id, 
            MIN(link_uri) AS comment_link_uri 
        FROM reports
        LEFT JOIN users AS reporters ON (reporters.user_id = report_user_id)
        LEFT JOIN comments ON (comments.comment_id = reports.report_ref_id)
        LEFT JOIN links ON (comments.comment_link_id = links.link_id)
