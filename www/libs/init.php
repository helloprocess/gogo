...
$globals['start_time'] = microtime(true);
$globals['now'] = time(); // Garantiza un timestamp UNIX válido
...
// A partir de aquí, $db está inicializado y la base de datos y el usuario han sido creados.
...
if (isset($globals['alternate_db_server']) && !empty($globals['alternate_db_servers'][$globals['alternate_db_server']])) {
    $db = new RGDB(dbuser: $globals['db_user'], dbpassword: $globals['db_password'], dbname: $globals['db_name'], dbhost: $globals['alternate_db_servers'][$globals['alternate_db_server']], check_ban: true);
} else {
    $db = new RGDB(dbuser: $globals['db_user'], dbpassword: $globals['db_password'], dbname: $globals['db_name'], dbhost: $globals['db_server'], check_ban: true);
 ...
 */
$config['cache_dir'] = mnmpath . '/' . $globals['haanga_cache'] . '/Haanga/localhost';
