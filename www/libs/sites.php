...
class SitesMgr
{
    ...
    public static function deploy($link)
    {
        ...
        if ($receivers) {
            foreach ($receivers as $r) {
                ...
                $category = !empty($new->category) ? $new->category : 0; // Cambia 'default' si es necesario
                ...
                $r = $db->query("
                    REPLACE INTO sub_statuses (id, status, date, link, origen, karma, category)
                    VALUES ($r, '$new->status', FROM_UNIXTIME($new->date), $new->link, $new->origen, $new->karma, '$category')
                ");
            }
        }
    ...
    public static function store_extended_properties(&$prefs, $id = false)
    {
        if ($id == false) {
            $id = self::my_id();
        }
