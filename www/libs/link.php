...
class Link extends LCPBase
{
    ...
    public $geo;
    public $thumb_uri;
    public $do_inline_friend_votes = false;
    public $key;
    public $author = -1;
    public $blog = 0;
    ...
    public static function getPromotedArticles($limit = 2)
    {
        ...
        $sql = '
            SELECT DISTINCT link, link_karma
            FROM sub_statuses, subs, links
            WHERE (
                link_content_type = "article"
                AND link_status IN ("queued", "published")
    ...
    public function store_basic($really_basic = false)
    {
        ...
        if (empty($this->randkey)) {
            $link_randkey = mt_rand(100000, 999999);
        }
        else $link_randkey = $this->randkey;
        // Asegurar que $link_published_date sea un número válido
        if (empty($link_published_date) || $link_published_date < strtotime('2000-01-01')) {
            $link_published_date = $link_sent_date;
        }
        ...
        if (empty($this->id)) {
            ...
            $r = $db->query('
                ...
                SET
                    ...
                    `link_content` = "'.$db->escape($link_content?? '').'",
    ...
    public function get_uri()
    {
        ...
        error_log("\033[38;5;208mDEBUG: Iniciando get_uri() con title: " . $this->title."\033[0m");

        require_once mnminclude.'uri.php';

        $new_uri = $base_uri = get_uri($this->title);
        error_log("\033[38;5;208mDEBUG: URI inicial generada: " . $new_uri."\033[0m");

        $seq = 0;

        // La URI no es igual a una URI estándar, de dispatch.php
        while (($seq < 20) && (!empty($routes[$new_uri]) || $db->get_var(
            ...
            error_log("\033[38;5;208mDEBUG: Iteración $seq, intentando URI: " . $new_uri."\033[0m");
        }

        // En caso de que se hayan intentado 20 veces, simplemente se agrega el id del artículo
        if ($seq >= 20) {
            $new_uri = $base_uri."-$this->id";
            error_log("\033[38;5;208mDEBUG: Se alcanzó el límite de iteraciones, URI final: " . $new_uri."\033[0m");
        }

        $this->uri = $new_uri;
        error_log("\033[38;5;208mDEBUG: get_uri() finalizó con URI: " . $this->uri."\033[0m");
    }

    public function get_short_permalink()
    {
        global $globals;
        error_log("\033[38;5;208mDEBUG: Iniciando get_short_permalink()\033[0m");

        if (isset($globals['url_shortener'])) {
            $server_name = $globals['url_shortener'].'/';
            $id = base_convert($this->id, 10, 36);
            error_log("\033[38;5;208mDEBUG: Usando URL shortener, server: " . $server_name . " y id convertido: " . $id."\033[0m");
        } else {
            $server_name = get_server_name().$globals['base_url'].'story/';
            $id = $this->id;
            error_log("\033[38;5;208mDEBUG: Usando URL estándar, server: " . $server_name . " y id: " . $id."\033[0m");
        }

        $short_permalink = $globals['scheme']."//".$server_name.$id;
        error_log("\033[38;5;208mDEBUG: Permalink corto generado: " . $short_permalink."\033[0m");
        return $short_permalink;
    }

    public function get_relative_permalink($strict = false)
    {
        global $globals;
        error_log("\033[38;5;208mDEBUG: Iniciando get_relative_permalink() con strict = " . ($strict ? "true" : "false")."\033[0m");

        if (empty($this->base_url)) {
            $this->base_url = $globals['base_url_general'];
            error_log("\033[38;5;208mDEBUG: Base URL vacía, usando base_url_general: " . $this->base_url."\033[0m");
        }
        ...
        if ($this->is_sub && ($globals['submnm'] || $strict || self::$original_status || !$this->allow_main_link)) {
            if (!empty($globals['submnm']) && $this->sub_status_id == SitesMgr::my_id() && !$strict && !self::$original_status) {
                ...
                error_log("\033[38;5;208mDEBUG: Ruta de submnm seleccionada: " . $base."\033[0m");
            } else {
                $base = $this->base_url.'m/'.$this->sub_name.'/';
                error_log("\033[38;5;208mDEBUG: Ruta de sub_name seleccionada: " . $base."\033[0m");
            }
        } elseif ($this->status === 'private') {
            $base = $this->base_url.'my-story/'.$this->username.'/';
            error_log("\033[38;5;208mDEBUG: Historia privada, usando ruta: " . $base."\033[0m");
        } else {
            $base = $this->base_url.'story/';
            error_log("\033[38;5;208mDEBUG: Usando ruta estándar: " . $base."\033[0m");
        }

        $relative_permalink = $base.(empty($this->uri) ? $this->id : $this->uri);
        // Si $relative_permalink comienza con $globals['server_name'], se quita esa parte
        if (strpos($relative_permalink, $globals['server_name']) === 0) {
            $relative_permalink = substr($relative_permalink, strlen($globals['server_name']));
            error_log("\033[38;5;208mDEBUG: Se quitó global server name, nuevo permalink relativo: " . $relative_permalink."\033[0m");
        } else {
            error_log("\033[38;5;208mDEBUG: global server name no encontrado al inicio del permalink."."\033[0m");
        }
        error_log("\033[38;5;208mDEBUG: Permalink relativo generado: " . $relative_permalink."\033[0m");
    ...
    {
        global $globals;
        error_log("\033[38;5;208mDEBUG: Iniciando get_permalink() con strict = " . ($strict ? "true" : "false")."\033[0m");

        if (empty($globals['server_name'])) {
            $server_name = $this->server_name;
            error_log("\033[38;5;208mDEBUG: server_name global vacío, usando: " . $server_name."\033[0m");
        } else {
            $server_name = $globals['server_name'];
            error_log("\033[38;5;208mDEBUG: Usando server_name global: " . $server_name."\033[0m");
        }

        if (empty($relative)) {
            $relative = $this->get_relative_permalink($strict);
            error_log("\033[38;5;208mDEBUG: Permalink relativo obtenido: " . $relative."\033[0m");
        }

        $permalink = $globals['scheme'].'//'.$server_name.$relative;
        error_log("\033[38;5;208mDEBUG: Permalink completo generado: " . $permalink."\033[0m");
        return $permalink;
    }
    ...
    {
        global $globals;
        error_log("\033[38;5;208mDEBUG: Iniciando get_canonical_permalink() con page: " . ($page ? $page : "false")."\033[0m");

        if (empty($globals['canonical_server_name'])) {
            $server_name = $this->server_name;
            error_log("\033[38;5;208mDEBUG: canonical_server_name global vacío, usando: " . $server_name."\033[0m");
        } else {
            $server_name = $globals['canonical_server_name'];
            error_log("\033[38;5;208mDEBUG: Usando canonical_server_name global: " . $server_name."\033[0m");
        }

        $page = (!$page || $page == 1) ? '' : "/$page";
        error_log("\033[38;5;208mDEBUG: Valor de page tras procesamiento: " . $page."\033[0m");

        $canonical_permalink = $globals['scheme'].'//'.$server_name.$this->get_relative_permalink(true).$page;
        error_log("\033[38;5;208mDEBUG: Permalink canónico generado: " . $canonical_permalink."\033[0m");
        return $canonical_permalink;
    }
