...
if (PHP_SAPI !== 'cli' && !empty($globals['force_ssl']) && !$globals['https'] && !isset($_GET['force'])) {
    ...
    die(header('Location: https://' . $_SERVER['SERVER_NAME'] .':' .$_SERVER['SERVER_PORT']. $_SERVER['REQUEST_URI']));
}
...
if (($globals['allow_partial'] || preg_match('/meneame/i', $_SERVER['HTTP_USER_AGENT']))) {
    if (!$globals['mobile']) {
        $globals['ads'] = false;
    }
...
function do_header($title, $id = 'home', $options = false, $tab_options = false, $tab_class = 'dropdown-menu menu-subheader')
{
    ...
    if ($this_site->sub) {
        ...
        //$this_site->url = $globals['base_url'];
    }
    ...
    return Haanga::Load('header.html', compact(
        'title', 'id', 'left_options', 'right_options',
...
function do_active_stories()
{
    ...
    $title = _('destacadas');
    $url = $globals['base_url'] . '---HTML1.php---DESTACADAS---'. 'top_active';
    $subclass = 'red';
...
function do_best_stories()
{
    ...
    $title = _('más votadas');
    $url = $globals['base_url'] . '---HTML1.php---MAS VOTADAS---'. 'popular';
    $subclass = 'red';
...
function do_best_queued()
{
    ...
    $title = _('candidatas');
    $url = $globals['base_url'] . '---HTML1.php---CANDIDATAS---'. 'queue?meta=_popular';
    $subclass = '';
...
function do_last_subs($status = 'published', $count = 10, $order = 'date')
{
    ...
    $key = "last_subs_$status-$count-$order" . $globals['v'];
    ...
    $title = _('En subs');
    $url = $globals['base_url_general'] . '---HTML1.php---EN SUBS---'. 'subs';
    $subclass = 'brown';
...
function do_sub_message_right()
{
    ...
    if ((isset($globals['mobile']) && $globals['mobile']) || !isset($globals['submnm']) || !$globals['submnm']) {
        return;
    }
