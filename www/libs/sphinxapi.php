...
class SphinxClient
{
    ...
    public function _Connect()
    {
        ...
        // Si no es un recurso, forzamos la reapertura:
        $this->_socket = false;
    ...
    public function AddQuery($query, $index="*", $comment="")
    {
        ...
        // Comprueba si $_weights es countable, si no, usa 0
        $weightsCount = (is_array($this->_weights) || $this->_weights instanceof Countable) 
                        ? count($this->_weights) 
                        : 0;
        $req .= pack("N", $weightsCount); // weights
        ...
        $min_id = (isset($this->_min_id) && is_numeric($this->_min_id)) ? $this->_min_id : 0;
        $max_id = (isset($this->_max_id) && is_numeric($this->_max_id)) ? $this->_max_id : 0;
        $req .= sphPackU64((int)$min_id) . sphPackU64((int)$max_id); // id64 range
        ...
        $indexWeights = (is_array($this->_indexweights)) ? $this->_indexweights : array();
        $req .= pack("N", count($indexWeights));
        foreach ($indexWeights as $idx => $weight) {
        ...
        // Usar un array vacío si $this->_overrides no es un array
        $overrides = is_array($this->_overrides) ? $this->_overrides : array();
        $req .= pack("N", count($overrides));

        foreach ($overrides as $key => $entry) {
            $req .= pack("N", strlen($entry["attr"])) . $entry["attr"];
            
            // Validar que 'values' sea un array, de lo contrario usar un array vacío
            $values = (isset($entry["values"]) && is_array($entry["values"])) ? $entry["values"] : array();
            $req .= pack("NN", $entry["type"], count($values));
