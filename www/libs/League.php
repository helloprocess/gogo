...
class League
{
    ...
    public function get_pages($per_page=2)
    {
        ...
        $max = $db->get_row("SELECT count(*) as total FROM " . GameMatch::TABLE . " WHERE league_id = $this->id");
        ...
        return $db->get_results("SELECT id FROM " . GameMatch::TABLE . " WHERE league_id = $this->id ORDER BY date DESC LIMIT {$current}, $per_page");
    }
