...
function favorite_exists($user, $link, $type = 'link')
{
    ...
    $type = $db->escape($type);
    return intval($db->get_var("SELECT COUNT(*) FROM favorites WHERE favorite_user_id=$user and favorite_type='$type' and favorite_link_id=$link"));
}
...
function favorite_get_readed($user, $link, $type = 'link')
{
    ...
    return intval($db->get_var("SELECT  favorite_link_readed FROM favorites WHERE favorite_user_id=$user and favorite_type='$type' and favorite_link_id=$link"));
}
...
function get_unread_favorites($user = 0, $type = 'link')
{
    ...
    return intval($db->get_var("SELECT COUNT(*) FROM favorites WHERE favorite_user_id=$user and favorite_type='$type' and favorite_link_readed=0"));
}
